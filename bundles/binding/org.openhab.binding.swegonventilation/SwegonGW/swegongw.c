 ...
 *
 *  7.9.2013	v1.00	Initial version
 *
 */
            ...
            while ((len = read(serialport_fd, buffer, MAX_DATA_LEN)) > 0)
            {
                ...
                for (int i = 0; i < len; i++)
                {
                    ...
                    if (start_found)
                    {
						...
						if (data_len > 0 && msg_len == (5 + data_len + 2) )
						{
							// whole frame received
							
							int calculatedCRC = crc16_ccitt( message, msg_len - 2 );
							int msgCRC = ((message[5 + data_len] << 8) & 0xFF00) + message[5 + data_len + 1];
							...
							if (verbose > 2) {
                    			...
                    			printData( " - data: ", &message[6], data_len - 2, "\n");
							...
							if (msgCRC == calculatedCRC)
							{
								if (verbose > 1) {
									...
									printData( ", data: ", message, 5 + data_len, "\n");
								}
								
								if (sendto(udp_fd, message, 5 + data_len + 1, 0 , (struct sockaddr *)&dest, sizeof(dest)) == -1)
								{
